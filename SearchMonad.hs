...
-- | Searching a doubleton {x, y} for an element that satisfies a given predicate p yields x if p(x) holds and must yield some element of {x,y} otherwise, therefore it returns y if p(x) does not hold.
doubleton :: a -> a -> S a
